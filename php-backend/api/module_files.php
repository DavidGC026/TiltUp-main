...
$method = $_SERVER['REQUEST_METHOD'];
    ...
    if (!$isAdmin) {
        json_response(['error' => 'No tiene permisos para eliminar archivos'], 403);
    }
    ...
    if (!$file_id) {
         // Try finding it in query params
    ...
    $stmt = $conn->prepare("SELECT file_path, preview_image_path FROM module_files WHERE id = ?");
    $stmt->bind_param("i", $file_id);
    $stmt->execute();
    $result = $stmt->get_result();
    
    if ($row = $result->fetch_assoc()) {
        $physical_path = ".." . $row['file_path']; // Convert relative web path to fs path
